...
function hasAnalyticsConsent() {
  return localStorage.getItem('wfh_analytics_consent') === 'true';
}
...
function setAnalyticsConsent(consent) {
  localStorage.setItem('wfh_analytics_consent', consent.toString());
}
...
function getSessionId() {
  let sessionId = sessionStorage.getItem('wfh_session_id');
  if (!sessionId) {
    sessionId = 'sess_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
    sessionStorage.setItem('wfh_session_id', sessionId);
  }
  return sessionId;
}
...
function autonomousTrackingSetup() {
  ...
  trackEvent('app_start', {
    first_open: !localStorage.getItem('wfh_first_open_done'),
    language: localStorage.getItem('wfh_lang') || 'de'
  });
  localStorage.setItem('wfh_first_open_done', 'true');
