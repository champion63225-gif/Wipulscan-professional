/**
 * WiFi Hunter - Fort Knox Security
...
class Tokenizer {
  ...
  loadTokenMap() {
    const stored = localStorage.getItem('wfh_token_map');
    if (stored) {
  ...
  saveTokenMap() {
    localStorage.setItem('wfh_token_map', JSON.stringify([...this.tokenMap]));
  }
...
class SecureStorage {
  ...
  getOrCreateEncryptionKey() {
    let key = localStorage.getItem('wfh_encryption_key');
    if (!key) {
      key = this.generateKey();
      localStorage.setItem('wfh_encryption_key', key);
    }
    return key;
  }
  ...
  setItem(key, value) {
    ...
    localStorage.setItem('wfh_secure_' + key, encrypted);
  }
   ...
   */
  getItem(key) {
    const encrypted = localStorage.getItem('wfh_secure_' + key);
...
class DataProtection {
  ...
  deleteAllUserData() {
    ...
    localStorage.removeItem('wfh_entitlements');
    localStorage.removeItem('wfh_history');
    localStorage.removeItem('wfh_token_map');
    localStorage.removeItem('wfh_encryption_key');
    
    // Delete secure storage
    Object.keys(localStorage)
      .filter(key => key.startsWith('wfh_secure_'))
      .forEach(key => localStorage.removeItem(key));

    // Delete analytics consent
    localStorage.removeItem('wfh_analytics_consent');
  }
  ...
  exportUserData() {
    const userData = {
      entitlements: JSON.parse(localStorage.getItem('wfh_entitlements') || '{}'),
      history: JSON.parse(localStorage.getItem('wfh_history') || '[]'),
      settings: {
        language: localStorage.getItem('wfh_lang'),
        consent: localStorage.getItem('wfh_consent'),
        onboardingDone: localStorage.getItem('wfh_onboarding_done')
      },
